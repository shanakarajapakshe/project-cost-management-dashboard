...
class DataManager {
    ...
    sanitizeProject(p) {
        const numFields = ["numEngineers","engineerSalary","ceVisitCharge","visitsPerMonth",
                           "transportCost","clientPayment","overheadAllocation"];
        const clean = { ...p };
        numFields.forEach(f => { clean[f] = parseFloat(clean[f]) || 0; });
        return this.calculateProjectMetrics(clean);
    }
    ...
    getSummaryStats() {
        ...
        const totalRevenue = this.projects.reduce((sum, p) => sum + (parseFloat(p.clientPayment) || 0), 0);
        const totalCosts = this.projects.reduce((sum, p) => sum + (parseFloat(p.totalCost) || 0), 0);
    ...
    saveToLocalStorage() {
        const period = this.getCurrentPeriod();
        // Guard: don't save to a null key if period was never set
        if (!this.currentMonth || !this.currentYear) {
            console.warn('Cannot save: period not set');
            return;
        }
    ...
    async loadFromLocalStorage() {
        ...
        if (this.isElectron) {
            ...
            if (result.success) {
                this.projects = (result.data || []).map(p => this.sanitizeProject(p));
            } else {
            ...
            if (data) {
                this.projects = JSON.parse(data).map(p => this.sanitizeProject(p));
            } else {
                this.projects = [];
            }
        }
...
class UIController {
    ...
    formatCurrency(amount) {
        const safe = isNaN(amount) ? 0 : (parseFloat(amount) || 0);
...
class AppController {
    ...
    initializeApp() {
        ...
        // Auto-set period from current month/year selects so saving works immediately
        ...
        // Auto-load current month data on startup
        this.loadMonthData();
    }
...
function setupUpdaterUI() {
    ...
    window.electronAPI.onUpdaterStatus((data) => {
        switch (data.type) {
            case 'checking':
                // Silent — don't show bar for routine checks
                break;
            case 'available':
                show(`🔔 Update v${data.version} available`, 'Download', () => {
                    window.electronAPI.checkForUpdates();
                    bar.style.display = 'none';
                });
                break;
            case 'downloading':
                show(`⬇️ Downloading update... ${data.percent}%`, null, null);
                break;
            case 'downloaded':
                show(`✅ Update v${data.version} ready — restart to install`, 'Restart Now', () => {
                    window.electronAPI.checkForUpdates(); // triggers quitAndInstall via main
                });
                break;
            case 'up-to-date':
                // Silent — no bar needed
                break;
            case 'error':
